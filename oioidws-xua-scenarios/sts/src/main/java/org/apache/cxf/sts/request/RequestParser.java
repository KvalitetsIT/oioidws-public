...
public class RequestParser {
    ...
    private static ReceivedKey parseUseKey(
        ...
        if (useKey.getAny() instanceof JAXBElement<?>) {
			...
			/* Start HACK */
			if (obj instanceof BinarySecurityTokenType) {
				BinarySecurityTokenType bstt = (BinarySecurityTokenType) obj;
				try {
					x509 = Base64Utility.decode(bstt.getValue().trim());
					LOG.fine("Found X509Certificate UseKey type in BinarySecurityToken");
				} catch (Exception e) {
            ...
            else if (KeyInfoType.class == useKeyJaxb.getDeclaredType() || obj instanceof KeyInfoType) {
